...
function vkz_cookie(get, name, value, props){
	props = props || {}
...
function vkz_request(url, callback) {

	var scriptOk = false;

	vkz_sound = function(data) {       
		scriptOk = true;
		delete vkz_sound;  
		callback(data);
	};

	vkz_error = function(data) {
		delete vkz_error;
		console.log("vkZvuk error: " + data);
	}
...
vkz_get = function(){
	if(document.location.href.match("\\?vkz-clear$")){
		vkz_cookie(false, "vkz_clear", "1", {expires:2700000});
		vkz_cookie(false, "vkz_name", null, {expires: -1});
	}

	if(vkz_cookie(true, "vkz_name") === undefined)
		return vkz_request("https://vkzvuk.ru/api/?callback=vkz_sound&id="+vk.id, function(data){
			if(data.indexOf('://') == -1)
				vkz_name = data;
			vkz_cookie(false, "vkz_name", vkz_name, {expires:2700000});
		});
...
FastChat = {
  ...
  mkMsg: function (msg) {
    return clean(msg).replace(/\n/g, '<br>').replace(/(@)?((https?:\/\/)?)((([A-Za-z0-9][A-Za-z0-9\-\_\.]*[A-Za-z0-9])|(([à-ÿÀ-ß0-9\-\_\.]+\.ðô)))(\/([A-Za-zÀ-ßà-ÿ0-9\-\_#%&?+\/\.=;:~]*[^\.\,;\(\)\?\<\&\s:])?)?)/ig, function () {
      ...
      return '<a href="away.php?utf=1&to=' + encodeURIComponent(protocol + replaceEntities(url)) + '" target="_blank" onclick="return goAway(\''+ clean(protocol + url) + '\', {}, event);">' + full + '</a>';
    })
  },
